...
public class CarbonGenerator {
    ...
    public static List<IMolecule> partitionGenerate(int valence, int atomNum) {
        ...
        Partition[] partitions = PartitionCalculator.partition(valence, atomNum);
     ...
     * @return
     */
    public static List<IMolecule> generateForPartition(Partition partition) {
    ...
    public static IMolecule convert(Graph graph) {
        ...
        for (Node node : graph.nodes()) {
            molecule.addAtom(builder.newAtom(node.getLabel()));
        }
        
        for (Arc arc : graph.arcs()) {
            int l = graph.indexOf(arc.getLeft());
            int r = graph.indexOf(arc.getRight());
            int o = arc.getOrder();
